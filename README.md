...
JSE is a local-first desktop assistant for finding jobs, scoring fit against one
or more resume lanes, managing the application pipeline, researching employers,
and generating tailored application material.
...
The app stores working data locally. Generated documents, settings, databases,
browser profiles, and backups should be treated as private user data.
...
If you are using a packaged build, dependencies may already be bundled. For
development or first source checkout, install both Node and Python dependencies.
...
These Local LLM values are stored in the settings directory as
`local_llm_settings.json`.
...
- JSON-mode support is strongly recommended because JSE asks for structured
  outputs in several workflows.
- Larger context windows improve analysis and document quality. A practical
  target is 16k-32k context or higher.
...
Do not put live API keys in source files. Store them through Settings or another
private local mechanism.
...
5. Run a small search to check selectors, login/session requirements, and
   duplicate handling.
...
- `settings/`: app settings, `local_llm_settings.json`, local database,
  browser/session profiles, context cache
...
Do not share these folders unless you intentionally want to share their contents.
...
- Try the external prompt workflow if the local model struggles with long JSON
  output.
